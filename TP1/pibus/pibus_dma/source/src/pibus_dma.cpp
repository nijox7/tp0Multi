...
void PibusDma::transition() {
    ...
    switch (r_master_fsm) {
        ...
        case DMA_READ_AD:
            ...
            if (r_index == r_max - 1) {
                r_master_fsm = DMA_READ_DT;
            }
            else {
                r_master_fsm = DMA_READ_DTAD;
            }
            break;
        case DMA_READ_DTAD:
        ...
        case DMA_WRITE_AD:
            ...
            if (r_index == r_max - 1) {
                r_master_fsm = DMA_WRITE_DT;
            }
            else {
                r_master_fsm = DMA_WRITE_DTAD;
            }
            break;
        case DMA_WRITE_DTAD:
            if (p_ack.read() == PIBUS_ACK_READY) {
...
void PibusDma::genMoore() {
    ...
    // p_req signal
    if ((r_master_fsm == DMA_READ_REQ) || (r_master_fsm == DMA_WRITE_REQ)) {
        p_req = true;
    }
    else {
        p_req = false;
    ...
    // p_d signal
    if ((r_master_fsm == DMA_WRITE_DTAD) || (r_master_fsm == DMA_WRITE_DT)) {
        p_d = (uint32_t) m_buf[r_index];
    }
