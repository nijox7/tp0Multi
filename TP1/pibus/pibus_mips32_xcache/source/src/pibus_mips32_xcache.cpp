...
void PibusMips32Xcache::transition()
{
    ...
    case DCACHE_IDLE :
    {
        ...
        // Processor request
            ...
            // WRITE request
                ...
                if (dcache_hit && dcache_cacheable) {
            ...
            // no valid m_dreq and no snoop request
            r_dcache_fsm = DCACHE_IDLE;
        }
        break;
    }
    ...
    if (m_snoop_active) {
        ...
        if (external_write) {
            ...
            if (r_snoop_dcache_inval_req.read() and (cache_hit  or wait_hit) and         
                ...
                if (r_dcache_fsm != DCACHE_IDLE) {
                    // we cannot handle the new external hit
            ...
            if (cache_hit) {
                ...
                // cache_hit and wait_hit cannot be simultaneously true
                r_snoop_dcache_inval_req = true;
            ...
            snoop_llsc_inval = (snoop_addr == r_llsc_addr.read() && r_llsc_pending.read()) || (m_dreq.valid && m_dreq.type == soclib::common::Iss2::DATA_LL && snoop_addr == m_dreq.addr);
            if (snoop_llsc_inval) {
                r_snoop_llsc_inval_req   = true;
            }
        }
    } // end if snoop_active
    ...
    case PIBUS_IDLE : 
    {
        ...
        else if (r_dcache_miss_req.read()) {
            // DMISS request
            r_pibus_ins  = false;
            r_pibus_addr = r_dcache_save_addr.read();
    ...
    case PIBUS_READ_AD :
    {
        ...
        if (r_pibus_opc == PIBUS_OPC_WDU) {
            r_pibus_fsm = PIBUS_READ_DT;
        }
        else {
            r_pibus_fsm = PIBUS_READ_DTAD;
        }
        break;
    }
    case PIBUS_READ_DTAD :
    {
        ...
        if ((p_ack.read() == PIBUS_ACK_READY) || (p_ack.read() == PIBUS_ACK_ERROR)) { 
            ...
            if (r_pibus_ins.read() and (r_pibus_wcount.read() == m_icache_words - 1)) {
                r_pibus_fsm = PIBUS_READ_DT;
            }
            if (!r_pibus_ins.read() and (r_pibus_wcount.read() == m_dcache_words - 1)) {
                r_pibus_fsm = PIBUS_READ_DT;
            }
        } 
        break;
